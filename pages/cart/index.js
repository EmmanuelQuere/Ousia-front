    ...
    <div className="flex shadow-md my-10">
      <div className="w-3/4 bg-white px-10 py-10">
        <div className="flex justify-between border-b pb-8">
          ...
          {!currentUserCart || <h2 className="font-semibold text-2xl">{totalQuantity(currentUserCart)} {totalQuantity(currentUserCart) > 1 ? `articles` : `article`}</h2>}
        </div>
        <div className="flex mt-10 mb-5">
          ...
          {!currentUserCart ? <span className="text-gray-600 text-lg">Vous n'avez pas encore d'articles dans votre panier !</span> : (!userToken ? currentUserCart.map(cartItem => <CartProduct key={cartItem.item.id} images={cartItem.item.images} product={cartItem.item} quantity={parseInt(cartItem.quantity, 10)} item_id={cartItem.item.id} setCurrentUserCart={setCurrentUserCart} currentUserCart={currentUserCart}/>) : currentUserCart.map(cartItem => <CartProduct key={cartItem.id} images={cartItem.images} product={cartItem.item} quantity={cartItem.quantity} item_id={cartItem.id} setCurrentUserCart={setCurrentUserCart} currentUserCart={currentUserCart}/>))}
          <Link href="/shop">
        ...
        {!currentUserCart || <div id="summary" className="w-1/4 px-8 py-10">
          ...
          <div>
            ...
            <select className="block p-2 text-gray-600 w-full text-sm" onChange={getSelectValue}>
              ...
              <option value="20">Livraison express - 20.00€</option>
            </select>
          </div>
