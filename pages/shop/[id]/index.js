      ...
      fetch(`${process.env.url}/cart_items.json`, {
        ...
        .then((response) => {
          if (response) {
            ...
            alert.show(`Une erreur est survenue, veuillez réessayer.`, {
              type: types.ERROR,
            });
          }
        })
        .catch((error) =>
          alert.show(`${error.message}`, { type: types.ERROR })
        );
    } else {
      let visitor_cart = JSON.parse(localStorage.getItem("visitor_cart")) || [];
      if (visitor_cart.find((element) => element.id === item.id)) {
        visitor_cart.find((element) => element.id === item.id).quantity =
          visitor_cart.find((element) => element.id === item.id).quantity +
          parseInt(quantity.current.value, 10);
      } else {
        visitor_cart.push({
          id: item.id,
          item: item,
            ...
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              ...
              <div className="flex mb-3">
                <a
                  ...
                  href="https://www.facebook.com/ousiadrinks"
                  role="button"
                >
                  <FontAwesomeIcon
                    icon={faFacebookF}
                ...
                <a
                  className="btn btn-outline-light btn-floating m-1"
                  href="https://www.linkedin.com/company/ousiadrinks/"
                  role="button"
                >
                  <FontAwesomeIcon
                    icon={faLinkedin}
                ...
                <a
                  className="btn btn-outline-light btn-floating m-1"
                  href="https://www.instagram.com/ousiadrinks/"
                  role="button"
                >
                  <FontAwesomeIcon
                    icon={faInstagram}
              ...
              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-200 mb-5">
                <div className="flex items-center">
                  ...
                  <span className="pl-3">
                    Lot(s) de 12 bouteilles de 27,5cL
                  </span>
                </div>
              </div>
              ...
              <div className="sm:flex text-center space-y-4 sm:space-y-0">
                ...
                <button
                  className="flex ml-8 sm:ml-auto text-white bg-blue-500 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 rounded"
                  onClick={addToCart}
                >
                  Ajouter au panier
                </button>
              </div>
              {item.description.split("/n/n")[1] ? (
                <dl>
                  <dt className="font-bold">Ingrédients :</dt>
                  <dd>{item.description.split("/n/n")[1]}</dd>
                </dl>
              ) : (
                ""
              )}
              {item.description.split("/n/n")[2] ? (
                <dl>
                  <dt className="font-bold">
                    Informations nutritionnelles pour 100ml :
                  </dt>
                  <dd>{item.description.split("/n/n")[2]}</dd>
                </dl>
              ) : (
                ""
              )}
